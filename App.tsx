...
// --- DADOS DO SITE (TEXTOS RESTAURADOS) ---
...
function App() {
  ...
  // --- LÓGICA: TEMPO PARA O DIA DOS NAMORADOS ---
  ...
  const toggleHeroAudio = () => {
    if (heroAudioRef.current) {
      heroIsPlaying ? heroAudioRef.current.pause() : heroAudioRef.current.play();
    ...
    <div className="min-h-screen bg-white font-sans text-gray-900 selection:bg-rose-100 selection:text-rose-900">
      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-lg bg-[#fffbf0] rounded-xl shadow-2xl border-4 border-white overflow-hidden">
            <button onClick={() => setShowPopup(false)} className="absolute top-2 right-2 z-20 bg-black/10 p-1 rounded-full"><X size={20} /></button>
            <div className="h-40 bg-slate-200 relative">
                <img src="https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=1000" alt="Paris" className="w-full h-full object-cover"/>
                <div className="absolute top-4 right-12 bg-white p-2 rotate-12 border-2 border-dotted border-slate-300 shadow-md"><Stamp className="text-rose-400 opacity-80" size={32} /></div>
            </div>
            <div className="px-8 pb-8 pt-2 text-center">
                <h2 className="text-3xl font-serif font-bold text-slate-900 mb-2">Ganha uma Viagem a <span className="text-rose-600 italic">Paris!</span></h2>
                <p className="text-slate-600 text-sm mb-6">A história de amor mais bonita das primeiras <strong className="text-slate-900">100 encomendas</strong> ganha voo + hotel para duas pessoas.</p>
                ...
                <Button onClick={startWizard} pulse fullWidth className="bg-slate-900 text-white py-4">Participar Agora</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* --- HEADER --- */}
      <header className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-rose-100 h-16">
        <div className="container mx-auto px-4 h-full flex items-center justify-between max-w-7xl">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView('landing')}>
            ...
            <span className="font-serif font-bold text-xl text-slate-800">Melodia do Amor</span>
          </div>
      ...
      <div className="md:hidden mt-16 bg-rose-50 py-2 flex justify-center border-b border-rose-100">
        <span className="text-[10px] font-bold text-rose-600 uppercase tracking-widest flex items-center gap-1">
          <Clock size={12} /> {tempoValentine}
        </span>
      </div>

      {/* --- HERO SECTION (TEXTO DA IMAGEM 28) --- */}
        ...
        <div className="container mx-auto px-4 max-w-6xl relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-20">
            <div className="flex-1 text-center lg:text-left space-y-4 lg:space-y-8 max-w-xl">
              <div className="inline-flex items-center gap-2 bg-rose-50 text-rose-600 px-4 py-1.5 rounded-full text-sm font-bold border border-rose-100 uppercase tracking-wide">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start pt-2">
                <Button onClick={startWizard} pulse className="w-full sm:w-auto px-8 py-4 text-lg bg-rose-600 hover:bg-rose-700 text-white shadow-xl">Criar a Minha Canção</Button>
                <div className="flex items-center gap-3 text-sm text-gray-500 font-medium">
                  ...
                  <div className="flex flex-col leading-none text-left"><span className="font-bold text-gray-900">+2500 casais</span><span className="text-xs">felizes</span></div>
                </div>
              </div>
            </div>
            <div className="flex-1 w-full max-w-[450px]">
              <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border-[6px] border-white">
                <img src={heroBg} alt="Casal" className="w-full h-auto object-cover aspect-[4/5]"/>
                <div className="absolute bottom-4 left-4 right-4">
                  <button onClick={toggleHeroAudio} className="w-full bg-white/95 backdrop-blur-xl rounded-2xl p-3 flex items-center gap-4 shadow-xl">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${heroIsPlaying ? 'bg-rose-500' : 'bg-rose-600'}`}>
                      {heroIsPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" className="ml-0.5" />}
                    </div>
                    <div className="text-left"><p className="font-bold text-sm text-gray-900 truncate">Exemplo: A Nossa História</p><p className="text-[10px] text-gray-500">{heroIsPlaying ? 'A Tocar...' : 'Ouvir Exemplo'}</p></div>
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* --- SOCIAL PROOF (TEXTO DA IMAGEM 29) --- */}
      ...
      {/* --- COMO CRIAMOS A MAGIA (TEXTO DA IMAGEM 30) --- */}
      <section className="py-24">
        <div className="container mx-auto px-4 max-w-6xl text-center">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-4">Como criamos a magia?</h2>
            <p className="text-gray-600 text-lg mb-16 max-w-xl mx-auto">Tu contas a história, nós transformamos em melodia. O processo é simples e rápido.</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl">
                    <div className="w-20 h-20 mx-auto bg-rose-50 rounded-full flex items-center justify-center text-rose-600 mb-6 border-4 border-white shadow-sm"><Heart size={32} /></div>
                    <h3 className="text-xl font-bold mb-3">1. Partilha a História</h3>
                    <p className="text-gray-600 text-sm">Preenche um formulário simples com os vossos nomes, memórias e o estilo musical que mais gostam.</p>
                </div>
                <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl">
                    <div className="w-20 h-20 mx-auto bg-rose-50 rounded-full flex items-center justify-center text-rose-600 mb-6 border-4 border-white shadow-sm"><Music size={32} /></div>
                    <h3 className="text-xl font-bold mb-3">2. Produção Profissional</h3>
                    <p className="text-gray-600 text-sm">Os nossos artistas e tecnologia compõem uma letra emocionante e uma melodia única baseada nos teus detalhes.</p>
                </div>
                <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl">
                    <div className="w-20 h-20 mx-auto bg-rose-50 rounded-full flex items-center justify-center text-rose-600 mb-6 border-4 border-white shadow-sm"><Gift size={32} /></div>
                    <h3 className="text-xl font-bold mb-3">3. Recebe em 72h</h3>
                    <p className="text-gray-600 text-sm">Recebe o ficheiro MP3 e a letra no teu e-mail. Pronta a oferecer e a emocionar!</p>
                </div>
            </div>
        </div>
      </section>

      {/* --- AUDIO SECTION (TEXTO DA IMAGEM 31) --- */}
              ...
              <ul className="space-y-6">
                {[
                  ...
                  <li key={i} className="flex gap-5">
                    ...
                    <div><h4 className="font-bold text-xl mb-1">{item.title}</h4><p className="text-gray-400 text-sm leading-relaxed">{item.desc}</p></div>
                  </li>
                ))}
              </ul>
              <div className="pt-6"><Button onClick={startWizard} className="py-4 px-8 text-lg bg-rose-600 hover:bg-rose-700 shadow-xl">Começar a Minha Música</Button></div>
            </div>
          </div>
        </div>
      </section>

      {/* --- PRICING SECTION (TEXTO IMAGEM 27 + 60% DESCONTO) --- */}
      <section id="pricing" className="py-24 bg-gradient-to-b from-white to-rose-50/50">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-rose-100 flex flex-col md:flex-row transform hover:scale-[1.01] transition-all duration-500">
            <div className="flex-1 p-8 md:p-16 flex flex-col justify-center bg-rose-600 text-white relative overflow-hidden">
               <div className="absolute inset-0 bg-gradient-to-br from-rose-500 to-rose-700"></div>
               <div className="relative z-10">
                 <h3 className="text-lg font-bold mb-2 text-rose-100 uppercase tracking-widest">Oferta Limitada</h3>
                 <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4 italic">Dia dos Namorados</h2>
                 <p className="text-rose-50 mb-8 text-lg leading-relaxed">Faltam apenas <span className="font-bold text-white underline">{diasFaltam} dias</span> para o Dia dos Namorados. Aproveita agora o preço promocional de lançamento!</p>
                 <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/20">
                   <div className="flex items-center gap-4">
                     <div className="bg-white/20 p-2 rounded-full"><Clock className="text-white" size={20} /></div>
                     <div>
            ...
            <div className="flex-1 p-8 md:p-16">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="bg-green-50 text-green-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase border border-green-100">60% Desconto</span>
              </div>
              ...
              <ul className="space-y-4 mb-10 text-gray-700">
                {["Música MP3 Completa (3-4 min)", "Letra 100% Personalizada", "Revisão Gratuita", "Entrega Standard (72h)", "Participação Concurso Paris"].map((feat, i) => (
                  <li key={i} className="flex items-center gap-3"><CheckCircle2 size={16} className="text-rose-600 shrink-0" /> <span>{feat}</span></li>
                ))}
              </ul>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 max-w-4xl text-center"><h2 className="text-3xl font-serif font-bold text-gray-900 mb-16">Perguntas Frequentes</h2><Faq items={FAQS} /></div>
      </section>

      <footer className="bg-gray-50 border-t border-gray-200 py-16">
        <div className="container mx-auto px-4 max-w-6xl flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex items-center gap-2"><div className="bg-rose-600 p-1.5 rounded-lg text-white"><Music size={20} fill="currentColor" /></div><span className="font-serif font-bold text-xl text-gray-900">Melodia do Amor</span></div>
            <div className="text-sm text-gray-500 flex gap-8 font-medium cursor-pointer"><span onClick={() => setView('terms')} className="hover:text-rose-600 transition-colors">Termos</span><span onClick={() => setView('privacy')} className="hover:text-rose-600 transition-colors">Privacidade</span><span onClick={() => setView('contact')} className="hover:text-rose-600 transition-colors">Contactos</span></div>
            <div className="text-xs text-gray-400">© {new Date().getFullYear()} Melodia do Amor Portugal.</div>
        </div>
      </footer>
      
      <div className="fixed bottom-4 left-4 right-4 z-40 md:hidden">
         <Button fullWidth className="shadow-2xl py-4 text-lg bg-rose-600 hover:bg-rose-700 text-white" onClick={startWizard}>Criar Música (24,99€)</Button>
      </div>
    </div>
  );
}
